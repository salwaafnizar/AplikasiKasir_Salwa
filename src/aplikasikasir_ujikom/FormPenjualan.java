    ...
    public FormPenjualan() {
     ...
     public void mulai(){
        ...
        private void simpan(){
             ...
             try {
                   String sql="insert into penjualasn (PenjualanID,DetailID,TanggalPenjualan,JamPenjualan,TotalHarga) value (?,?,?,?,?)";
           ...
           try{
               String sql = "SELECT MAX(RIGHT(PenjualanID,3)) AS NO FROM penjualasn";
               pst=konek.prepareStatement(sql);
               rst=pst.executeQuery();
               while (rst.next()) {
                       if (rst.first() == false) {
